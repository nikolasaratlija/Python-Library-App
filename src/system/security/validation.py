...
is_integer = ("^[0-9]+$", "Error: Input must be an integer.")

is_username = ("^[a-zA-Z][a-zA-Z0-9_'.]{5,10}$",
               "Error: Incorrect username format. username must be between 6 and 10 characters.")

is_password = ("^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!#%*?&]{8,18}$",
...
is_phone_number = ("^[0-9]{6}$", "Error: Incorrect phone format. Format must be DDDDDD")

is_email = ("^[a-z0-9]+[\._]?[a-z0-9]+[@]\w+[.]\w{2,3}$", "Error: Incorrect email format. Example: sample@email.com")

is_zipcode = ("^[0-9]{4}[A-Z]{2}$", "Error: Incorrect zipcode format. Format must be DDDDXX")

is_not_empty_or_whitespace = ("^$|\s+", "Error: Input is empty. Please enter a valid input")
